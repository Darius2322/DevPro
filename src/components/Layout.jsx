...
import {
  ...
  Bot, Plug, FolderOpen, User, MoreHorizontal, X
} from 'lucide-react'
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <aside
        ...
        style={{ width: 220, borderRight: '1px solid var(--border)', padding: 16, display: 'flex', flexDirection: 'column', gap: 4 }}
        data-desktop-only
      >
      ...
      <main className="scrollbar-thin" style={{ flex: 1, overflowY: 'auto', paddingBottom: 72 }}>
        <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 4, padding: '12px 20px 0' }}>
          <GlobalSearchButton />
          <ThemeToggle />
          <NotificationsBell />
        </div>
      ...
      {moreOpen && (
        ...
        >
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'var(--surface)', borderTop: '1px solid var(--border)', borderRadius: '12px 12px 0 0', padding: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <strong style={{ fontSize: 14 }}>More</strong>
              <button className="icon-btn" onClick={() => setMoreOpen(false)}><X size={16} /></button>
            </div>
            {SECONDARY_NAV.map(({ to, label, icon: Icon }) => (
              <NavLink key={to} to={to} onClick={() => setMoreOpen(false)} style={navLinkStyle}>
                <Icon size={16} />
                {label}
              </NavLink>
            ))}
            <button className="btn-ghost" style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'center', marginTop: 12 }} onClick={signOut}>
              <LogOut size={14} /> Sign out
            </button>
          </div>
        </div>
