...
export default function FilesPanel({ projectId }) {
  const { push } = useToast()
  ...
  const uploadFiles = useCallback(async (fileList) => {
    ...
    for (const file of fileList) {
      ...
      if (upErr) {
        push(`Upload failed: ${file.name}`, 'danger')
        continue
      }
      ...
      await logActivity({ projectId, action: 'File uploaded', detail: file.name })
    }
    setUploading(false)
  ...
  async function removeFile() {
    ...
    await supabase.from('files').delete().eq('id', toDelete.id)
    await logActivity({ projectId, action: 'File deleted', detail: toDelete.name })
    setToDelete(null)
    load()
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
        <h2 style={{ fontSize: 15, margin: 0 }}>Files</h2>
      ...
      {files?.length === 0 && <EmptyState title="No files yet" description="Upload your first project file." />}
            ...
            {files.map((f) => (
              <tr key={f.id}>
                <td style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <FileText size={14} color="var(--text-faint)" /> {f.name}
                </td>
                ...
                <td>
                  <div style={{ display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
                    <button className="icon-btn" title="Download" onClick={() => downloadFile(f)}><Download size={14} /></button>
                    <button className="icon-btn" title="Delete" onClick={() => setToDelete(f)}><Trash2 size={14} /></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      <ConfirmDialog open={!!toDelete} onClose={() => setToDelete(null)} onConfirm={removeFile} title="Delete file" description={`Delete "${toDelete?.name}"? This can't be undone.`} />
    </div>
  )
}
